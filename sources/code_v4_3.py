...
# Koordinaten von BerlinMitte
coordinatesMitte = (52.524436, 13.409616)

# # Import der Datei, die mit "FillingFeatures.py gefüllt wurden
test_feature_df = pd.read_csv(r'test_feature_df.csv')
test_label_df = pd.read_csv(r'test_label_df.csv')
test_df = pd.read_csv(r'test_df.csv')
train_feature_df = pd.read_csv(r'train_feature_df.csv')
train_label_df = pd.read_csv(r'train_label_df.csv')
...
# Gibt einen Score zur Reachability für eine bestimme Adresse zurück
...
# Berechnet die features für die gewünschte Adresse, um für die Adresse zu predicten.
def prediction(latitude, longitude, price, category):
    ...
    distanceToMitte = geopy.distance.geodesic(coordinates1, coordinatesMitte).m
    ...
    predscore = regr.predict([[price, others, own, vietnamese, sushi, pubs, pizza, kebab, italian,
                               icecream, hotdogs, german, divebars, cocktailbars, cafes, burgers, bars, bakeries,
                               distanceToMitte]])

    return (round(predscore[0][0], 2))


# Gibt eie Dataframe mit nur "Nachbarn" für Latitude,Longitude zurück -> soll prediction optimieren
def gettingneighbours(latitude, longitude):
    ...
    for gettingneighboursx in range(len(train_feature_df)):
        coordinates2 = train_feature_df.at[gettingneighboursx, 'latitude'], train_feature_df.at[
            gettingneighboursx, 'longitude']
        dist = geopy.distance.geodesic(coordinates1, coordinates2).m
        if (dist < radius):
...
# Zum Testen mit Statikstiken für die Accuracy
def test(test_df):
    ...
    print('Dif:')
    print(
        'Dif von PredictedScore:' + str(np.max(ergebnis_df['predictedscore']) - np.min(ergebnis_df['predictedscore'])))
    ...
    print('Spannweite Prediction: ' + str(
        (np.min(ergebnis_df['predictedscore'] - np.max(ergebnis_df['predictedscore'])))))
    # exportiert den testfall
    ergebnis_df.to_csv("ergebnis_df_onlyDistanceToMitte.csv", sep=",", encoding="utf-8")
    print(ergebnis_df)
